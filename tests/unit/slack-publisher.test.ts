...
import type { SlackInlineResponse } from '../../src/services/reporting/slack-publisher';
...
function makeLogger() {
  return { warn: vi.fn(), error: vi.fn(), info: vi.fn() } satisfies Pick<Logger, 'warn' | 'error' | 'info'>;
}

/** Typed shape for an arbitrary Slack block — just enough to assert on. */
...
describe('SlackPublisher', () => {
  ...
  describe('post', () => {
    it('POSTs Block Kit JSON to the webhook URL', async () => {
      ...
      const publisher = new SlackPublisher({ webhookUrl: 'https://hooks.slack.com/x', logger });
      ...
      const body = JSON.parse(init.body as string) as SlackInlineResponse;
    ...
    it('logs info when called with empty webhookUrl rather than fetching', async () => {
      ...
      const logger = makeLogger();
      const publisher = new SlackPublisher({ webhookUrl: '', logger });
      await publisher.post(sampleReport);
  ...
  describe('formatInline', () => {
    it('returns a Slack ephemeral response shape', () => {
      const publisher = new SlackPublisher({ webhookUrl: 'https://hooks.slack.com/x', logger: makeLogger() });
    ...
    it('post and formatInline produce identical block structure', async () => {
      ...
      const postBody = JSON.parse(postInit.body as string) as SlackInlineResponse;
  ...
  describe('formatting', () => {
    it('formats bytes in a human-readable unit', () => {
      const publisher = new SlackPublisher({ webhookUrl: '', logger: makeLogger() });
      const inline = publisher.formatInline(sampleReport);
    ...
    it('omits the new-payers context block when there are zero first-time payers', () => {
      ...
      const inline = publisher.formatInline(report);
      const hasContext = (inline.blocks as SlackBlock[]).some((b) => b.type === 'context');
