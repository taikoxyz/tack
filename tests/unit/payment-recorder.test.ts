...
describe('PaymentRecorder', () => {
  ...
  let logger: Pick<Logger, 'error' | 'warn'>;

  beforeEach(() => {
    db = createDb(':memory:');
    repo = new PaymentRepository(db);
    logger = { error: vi.fn(), warn: vi.fn() } satisfies Pick<Logger, 'error' | 'warn'>;
    recorder = new PaymentRecorder(repo, logger, () => '2026-04-21T12:00:00.000Z');
  });
  ...
  it('records a complete payment result and lowercases addresses', () => {
    ...
    const rows = db.prepare('SELECT * FROM payments').all() as PaymentRecord[];
  ...
  it('records null for tx_hash and pin_request_id when not provided', () => {
    ...
    const row = db.prepare('SELECT tx_hash, pin_request_id FROM payments').get() as Pick<PaymentRecord, 'tx_hash' | 'pin_request_id'>;
    expect(row.tx_hash).toBeNull();
    expect(row.pin_request_id).toBeNull();
  });

  it('logs error and swallows DB exceptions (does not throw)', () => {
    const failingRepo = { insert: vi.fn().mockImplementation(() => { throw new Error('disk full'); }) } satisfies Pick<PaymentRepository, 'insert'>;
    const r = new PaymentRecorder(failingRepo as unknown as PaymentRepository, logger, () => '2026-04-21T12:00:00.000Z');
  ...
  it('uses the injected clock for occurred_at', () => {
    ...
    const r = new PaymentRecorder(repo, logger, clock);
    r.record(fullResult, { requestId: 'req_7' });
    const row = db.prepare('SELECT occurred_at FROM payments').get() as Pick<PaymentRecord, 'occurred_at'>;
